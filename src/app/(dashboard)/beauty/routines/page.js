...
export default function RoutinesListPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ["routines"], queryFn: routinesApi.list });
  const items = data || [];
        ...
        render: (row) => (
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <p dir="ltr" className="truncate text-xs text-[var(--text-faint)]">/beauty/routines/{row.slug}</p>
            </div>
          </div>
        ),
      },
      { key: "steps", header: "تعداد مراحل", render: (row) => row.steps?.length || 0 },
      {
        key: "status",
        header: "وضعیت",
        render: (row) => (
      ...
      },
      {
        ...
        render: (row) => (
          <div className="flex gap-0.5" onClick={(e) => e.stopPropagation()}>
            <Button variant="ghost" size="icon" title="ویرایش" onClick={() => router.push(`/beauty/routines/${row._id}`)}>
              <Pencil size={14} />
            </Button>
  ...
  return (
    <div>
      <PageHeader
        title="قالب‌های روتین"
        subtitle="مدیریت روتین‌های آماده‌ای که در روتین‌ساز به مشتری پیشنهاد می‌شود"
        actions={
          <Button onClick={() => router.push("/beauty/routines/new")}>
            <Plus size={15} />
            روتین جدید
          </Button>
        }
      ...
      <DataTable
        ...
        data={items}
        isLoading={isLoading}
        emptyMessage="هنوز قالب روتینی ثبت نشده است"
        onRowClick={(row) => router.push(`/beauty/routines/${row._id}`)}
      />

      <ConfirmDialog
        open={!!deleteTarget}
        onOpenChange={(open) => !open && setDeleteTarget(null)}
        title="حذف قالب روتین"
