...
export default function IngredientsListPage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => ingredientsApi.remove(id),
    onSuccess: () => {
        ...
        render: (row) => (
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <p dir="ltr" className="truncate text-xs text-[var(--text-faint)]">/beauty/ingredients/{row.slug}</p>
            </div>
          </div>
        ),
      },
      { key: "relatedSkinTypes", header: "انواع پوست مرتبط", render: (row) => row.relatedSkinTypes?.length || 0 },
      {
        key: "status",
        header: "وضعیت",
        render: (row) => (
      ...
      },
      {
        ...
        render: (row) => (
          <div className="flex gap-0.5" onClick={(e) => e.stopPropagation()}>
            <Button variant="ghost" size="icon" title="ویرایش" onClick={() => router.push(`/beauty/ingredients/${row._id}`)}>
              <Pencil size={14} />
            </Button>
  ...
  return (
    <div>
      <PageHeader
        title="ترکیبات"
        subtitle="مدیریت صفحات ترکیبات آرایشی-بهداشتی در Beauty Knowledge Hub"
        actions={
          <Button onClick={() => router.push("/beauty/ingredients/new")}>
            <Plus size={15} />
            ترکیب جدید
          </Button>
        }
      ...
      <DataTable
        ...
        data={items}
        isLoading={isLoading}
        emptyMessage="هنوز ترکیبی ثبت نشده است"
        onRowClick={(row) => router.push(`/beauty/ingredients/${row._id}`)}
      />
